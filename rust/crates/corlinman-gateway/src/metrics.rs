...
pub use corlinman_core::metrics::{
    ...
    CHANNELS_RATE_LIMITED, CHAT_STREAM_DURATION, EVOLUTION_PROPOSALS_APPLIED,
    EVOLUTION_PROPOSALS_DECISION, EVOLUTION_PROPOSALS_LISTED, EVOLUTION_SIGNALS_DROPPED,
    EVOLUTION_SIGNALS_OBSERVED, EVOLUTION_SIGNALS_QUEUE_DEPTH, FILE_FETCHER_BYTES_TOTAL,
    FILE_FETCHER_FETCHES_TOTAL, HOOK_EMITS_TOTAL, HOOK_SUBSCRIBERS_CURRENT, HTTP_REQUESTS,
    LOG_FILES_REMOVED, PLUGIN_EXECUTE_DURATION, PLUGIN_EXECUTE_TOTAL, PROTOCOL_DISPATCH_ERRORS,
...
pub fn init() {
    ...
    EVOLUTION_PROPOSALS_DECISION
        .with_label_values(&["startup"])
        .inc_by(0.0);
    EVOLUTION_PROPOSALS_APPLIED.inc_by(0.0);
    LOG_FILES_REMOVED
        .with_label_values(&["startup"])
        .inc_by(0.0);
}
